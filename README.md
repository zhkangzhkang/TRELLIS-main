...
### Linux Server Deployment Guide (Chinese)

For users deploying TRELLIS on a remote Linux server, see the detailed Chinese guide: [readmev2.md](readmev2.md).

It covers a full beginner-friendly workflow for cloning with submodules, creating a Conda environment, matching `torch==2.4.0` with CUDA 12.1, fixing `CUDA_HOME`/`nvcc`/GCC issues, using `xformers` as the attention backend, installing `spconv`, `kaolin`, `nvdiffrast`, `diffoctreerast`, and `diff_gaussian_rasterization`, handling GitHub API rate limits for `torch.hub.load`, running `example.py`, understanding the generated `.mp4`/`.glb`/`.ply` files, and launching the Gradio demo.
